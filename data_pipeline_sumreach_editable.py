...
Function Process_sngl:
    Variables:
        ...
        --MainStarName=False; string:
            + If AIJ was not used or different information is wanted, this can beused to force theprogram to look at different data.
        --XFit=False; string:
            +The data in the csv to be used as the X-axis data.
    return: 
...
"""
    ...
    if MainStarName == False:
        ...
        if XFit == False:
            time_line, moving_median = median_fit(
                csv_full, y_data=MainStarName)
        else:
            time_line, moving_median = median_fit(
                csv_full, y_data=MainStarName, x_data=XFit)
        curve = np.polyfit(time_line, moving_median, deg=2)
        plt.plot(
            time_line, curve[0] * time_line ** 2 +
            curve[1] * time_line + curve[2], "b-"
...
Function Process_mult:
    Variables:
        ...
        --MainStarName=False; string:
            +If AIJ was not used or different information is wanted, this can beused to force theprogram to look at different data.
        --CompStarName=False; string or tuple:
            +Overwright the naming convention of the program. Either use a string to use naming pattern, if none exists, a tuplle can be used with each name used as the entries.
    Return:
...
"""
    ...
    if MainStarName == False:
        ...
        if isinstance(CompStarName, str):
            for time in range(len(main_star)):
                for star in range(2, comp_stars + 2):
                    avg_star[time] += csv_full[f"{CompStarName}{star}"][time]
                    continue
                continue
        elif isinstance(CompStarName, tuple):
            for time in range(len(main_star)):
                for star in range(len(CompStarName)):
                    avg_star[time] += csv_full[f"{CompStarName[star]}"][time]
                    continue
                continue
        else:
            for time in range(len(main_star)):
                for star in range(2, comp_stars+2):
                    avg_star[time] += csv_full[f"rel_fulx_SNR_C{star}"][time]
                    continue
                continue
        if style == "mean":
            avg_star /= comp_stars
        ...
        if XFit == False:
            ...
            time_line, moving_median = median_fit(
                csv_full, version=1, y_data=ratio_main_avg, x_data=XFit)
        print(np.shape(time_line))
        curve = np.polyfit(time_line, moving_median, deg=2)
        plt.plot(
            time_line, curve[0] * time_line ** 2 +
            curve[1] * time_line + curve[2], "b-"
        ...
        csv_full = pd.read_csv(file_position)
        main_star = csv_full[f"{MainStarName}"]
        avg_star = np.zeros(len(main_star))
        if isinstance(CompStarName, str):
            for time in range(len(main_star)):
                for star in range(2, comp_stars + 2):
            ...
            for time in range(len(main_star)):
                for star in range(2, comp_stars+2):
                    avg_star[time] += csv_full[f"rel_fulx_SNR_C{star}"]
                    continue
                continue
        if style == "mean":
            avg_star /= comp_stars
        ...
        if XFit == False:
            ...
            time_line, moving_median = median_fit(
                csv_full, version=1, y_data=ratio_main_avg, x_data=f"{XFit}")
        curve = np.polyfit(time_line, moving_median, deg=2)
        plt.plot(
            time_line, curve[0] * time_line ** 2 +
            curve[1] * time_line + curve[2], "b-"
...
"""
    ...
    if MainStarName == False:
        ...
        if XFit == False:
            ...
            time_line_single, moving_median_single = median_fit(
                csv_full, x_data=XFit)
        ...
        moving_median_single = pd.DataFrame(
            data=star_normalize, columns=["set"])
...
Process_mlt(r"C:\Users\guswo\Documents\Astronomy Pictures\08_08_2022 - WASP-80\csv\Measurements.csv",
            custom_name="0003_edit_test")
