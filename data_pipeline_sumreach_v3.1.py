...
def median_fit(dataframe,version=0,x_data='X(FITS)_T1',y_data='rel_flux_SNR_T1',save_graph=False,name='test_graph'):
...
def do_everything_mult(file_position,comp_stars=0,style='sum',filename=r'ratio_movin_median_plot',filetype=r'.png',nameaslist=False,tupleofname=()):
    csv_full=pd.read_csv(file_position)
    if nameaslist==False:
        main_star=csv_full['rel_flux_SNR_T1']
    elif nameaslist==True:
        main_star=csv_full[tupleofname[0]]
    ...
    if nameaslist==False:
        ...
        for time in range(1,len(tupleofname)):
            for star in range(2,comp_stars+2):avg_star[time]+=csv_full[f'{tupleofname[time]}{star}'][time]
            if style=='mean':avg_star[time]/=comp_stars
            continue
    ...
    plt.plot(time_line,moving_median,'k*')
    # instead of showing and stopping the program try and save it instead
    plt.savefig(f'{filename}_{style}{filetype}')
    print(f'\n\nsaved as {filename}\n')
    return 'done'
...
tumple=('rel_flux_SNR_T1','rel_flux_SNR_C2',"rel_flux_SNR_C3",'rel_flux_SNR_C4')
def do_everything_stack(file_position,comp_stars=0,style='sum',custom_name=r'ratio_movin_median_plot_stacked',filetype=r'.png'):
    ...
    return 'done'
do_everything_mult(r'C:\Users\guswo\Documents\Astronomy Pictures\06_09_2022 - WASP-3\csv\Measurements.csv',filename='testing_custom_list_imp',nameaslist=True,tupleofname=tumple)